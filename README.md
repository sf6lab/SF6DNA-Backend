...
`better-sqlite3`はネイティブビルドが必要なパッケージです。Renderは`package.json`の`engines.node`に**上限のないバージョン範囲**（例：`>=18`）を指定すると、その時点の最新Node.jsを使ってしまい、ネイティブモジュールのビルドに失敗することがあります。`engines.node`は`20.x`のように**上限付き**で指定してください（このプロジェクトは既に対応済みです）。
